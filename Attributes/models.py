...
class Book(Model):
    ...
    file1 = FileField(upload_to=join(settings.DATABASE_BOOK_DIR))
    file2 = FileField(upload_to=join(settings.DATABASE_BOOK_DIR), blank=True, null=True)
    file3 = FileField(upload_to=join(settings.DATABASE_BOOK_DIR), blank=True, null=True)
